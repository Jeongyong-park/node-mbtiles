...
function MBTiles(uri, callback) {
  ...
  try {
    mbtiles._db = new sqlite3(mbtiles.filename); // mode, function(err) {
...
MBTiles.prototype.getTile = function (z, x, y, callback) {
  ...
  var mbtiles = this;
  try {
    const row = this._db.prepare(sql).get(z, x, y);
    if (!row) {
    ...
    if (err && err.message.indexOf("no such table") < 0) callback(err);
  }
};
...
MBTiles.prototype.getGrid = function (z, x, y, callback) {
  ...
  var mbtiles = this;
  try {
    const row = mbtiles._db.prepare(sqlgrid).get(z, x, y);
    ...
    zlib.inflate(
      ...
      function (err, buffer) {
        ...
        try {
          const rows = mbtiles._db.prepare(sqljson).all(z, x, y);
...
MBTiles.prototype.getInfo = function (callback) {
  ...
  try {
    const rows = mbtiles._db.prepare("SELECT name, value FROM metadata").all();
    ...
    //if (err && err.errno !== 1) return callback(err);
    if (err && err.message.indexOf("no such table") < 0) return callback(err);
  }
  ...
  function ensureZooms(info, callback) {
    ...
    try {
      ...
      if (err && err.message.indexOf("no such table") < 0) {
        return done(err);
      } else {
        done(null, info);
      }
...
MBTiles.prototype.startWriting = function (callback) {
  ...
  mbtiles._setup(function (err) {
    ...
    try {
      mbtiles._db.pragma("synchronous=OFF");
      callback();
    } catch (err) {
      callback(err);
    }
...
MBTiles.prototype.stopWriting = function (callback) {
  ...
  mbtiles._commit(function (err) {
    if (err) return callback(err);
    try {
      mbtiles._db.pragma("synchronous=NORMAL");
